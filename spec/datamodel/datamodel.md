...
| **ViewPreference** | `id` (PK), `householdId` (FK), `view` (`PLAN` \| `SHOPPING` \| `REPORTS`), `widgetKey` (string — stable identifier per widget), `settings` (JSON — UC-012 shape: `{ "query": "<SELECT …>" }` for the leaderboard, `{ "query", "controllerStateB64" }` for charts), `updatedAt` | Belongs to `Household`. One row per `(household, view, widget)`. |
