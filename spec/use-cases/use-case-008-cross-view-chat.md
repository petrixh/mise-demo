...
| BR-07 | The orchestrator processes one turn at a time. A prompt submitted while a turn is in flight is **not queued or sent** — it is suppressed and the user is shown a transient "busy" note; typing "stop" cancels the in-flight turn. See **UC-013** for the full busy/stop behaviour. |
